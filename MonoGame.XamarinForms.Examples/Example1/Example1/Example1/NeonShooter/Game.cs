...
namespace NeonShooter
{
    ...
    public class NeonShooterGame: Microsoft.Xna.Framework.Game
    {
        ...
        Vector2 baseScreenSize = new Vector2(1280, 768);
        ...
        public NeonShooterGame()
        {
			...
			Content.RootDirectory = "NeonShooter";
            ...
            bloom.Settings = BloomSettings.PresetSettings.FirstOrDefault(s => s.Name == "Soft"); // new BloomSettings(null, 0.25f, 4, 2, 1, 1.5f, 1);
        ...
        {
            if (ScreenSize.X < 10 || ScreenSize.Y < 10) return;

            if (LastBounds != Viewport.Bounds)
            {
                // Rebuild the Grid
                const int maxGridPoints = 1600;
                Vector2 gridSpacing = new Vector2((float)Math.Sqrt(Viewport.Width * Viewport.Height / maxGridPoints));
